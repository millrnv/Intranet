...
public class CarreraDAO {
    public static void agregarCarrera(DSLContext query, Carrera carrera){
        ...
        query.insertInto(tablaCarrera, columnas[0], columnas[1],columnas[2]).values(carrera.getNombreCarrera(),carrera.getCodCarrera(),carrera.getCantSemestres()).execute();
    }
    public static boolean validarExistenciaCarrera(DSLContext query,String columnaTabla, Object dato){
        Result resultados = query.select().from(table("Carrera")).where(DSL.field(columnaTabla).eq(dato)).fetch();
